...
async function run() {
  try {
    ...
    // forget pass verification
    // ----------------------------------------------------------------------------------------

    app.post('/api/user/phoneNumber', async (req, res) => {

      try {
        const { phoneNumber } = req.body;
        const result = await UserCollection.findOne({ mobileNumber: phoneNumber });
        if (result) {
          res.status(201).send({ data: result, message: 'found' });
        } else {
    ...
    app.put('/api/user/password', async (req, res) => {
      try {
        const { mobileNumber, pass1 } = req.body;
        const result = await UserCollection.updateOne({ mobileNumber: mobileNumber }, { $set: { password: pass1 } });
        res.status(201).send({ message: "updated", data: result });
      } catch (error) {
        console.error('Error:', error);
        res.status(500).send('Internal server error');
      }
